...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['submitinformation'])) {
    ...
    try {
        ...
        // 1. Update Personal Data (Updated with specific address columns)
        $sqlA = "UPDATE personal_data SET 
                    lastname=?, firstname=?, middlename=?, suffix=?, dob=?, 
                    gender=?, civilstatus=?, civil_status_reason=?, tax_number=?, 
                    nationality=?, religion=?, pob_city=?, pob_country=?, 
        ...
        // 2. Update Beneficiaries (Keep as is)
        ...
        $pdo->prepare($sqlD)->execute([
            $data['SSSno'] ?? '', $data['businesscode'] ?? '', $data['approvedmsc'] ?? 0, $data['monthlysscontribution'] ?? 0, !empty($data['startofpayment']) ? $data['startofpayment'] : null, $data['workingspouseMSC'] ?? 0, $data['flexiapplication'] ?? null, !empty($data['dateandtime1']) ? $data['dateandtime1'] : null, !empty($data['dateandtime2']) ? $data['dateandtime2'] : null, !empty($data['dateandtime3']) ? $data['dateandtime3'] : null, $reg_id
        ]);

        $pdo->commit();
        echo "<script>alert('Record updated successfully!'); window.location='view_list.php';</script>";
    } catch (Exception $e) {
        if ($pdo->inTransaction()) $pdo->rollBack();
        echo "<script>alert('Update Error: " . addslashes($e->getMessage()) . "'); window.history.back();</script>";
    }
}
?>